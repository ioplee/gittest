...
function get_data(_biz_type, _no, _done_callback, _fail_callback, _always_callback){
    ...
    $.getJSON('/school/V1/selectOption/_findSelect', params).done(function(_data, _status, _xhr){
        if(parseInt(_data['code'], 10)===0 && _data['success']){
            // update local dict
...
function check_local_status(_biz_type, _no){
    var biz_type = _biz_type + '',
        no = _no + '';
    return (biz_type==='building') ? ((SOURCE_DICT[biz_type].length===0) ? false : true) : (_no !== null && !!SOURCE_DICT[biz_type][no]) ? true : false;
}
...
exports.find = function(_params, _callback){
    var biz_type = _params['bizType'] + '',
        no = (_params['no']===undefined) ? '' : _params['no'] + '';
