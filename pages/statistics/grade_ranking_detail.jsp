...
<head>
    ...
    <title>综合报表-班级寝室评分排名报表-报表明细</title>
    ...
    <div class="body-container">
        ...
        <div class="body-container__main">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-split">/</span>
                <span class="breadcrumb-active">班级寝室评分排名报表</span>
                <span class="breadcrumb-split">/</span>
            ...
            <div class="body-content__wrapper">
                <div class="ui-panel">
                    <div class="panel__header">
                        ...
                        <span class="panel-title"><i class="fa fa-list"></i>&nbsp;班级寝室评分报表明细</span>
                    </div>
                    <div class="panel__body">
                        <div class="ui-table-container">
                            <table class="ui-table" id="grade_ranking">
                                <thead>
                                    <tr>
                                        <th style="width:30px;">序号</th>
                                        <th style="width:160px">扣分日期</th>
                                        <th>学生姓名</th>
                                        <th>校内号</th>
                                        <th>寝室</th>
                                        <th style="width:60px">扣分</th>
                                        <th style="width:180px">扣分原因</th>
                                        <th style="width:120px">证据图片</th>
                                    </tr>
                                </thead><!-- 表头可选 -->
                                <tbody></tbody>
                            </table>
                        </div>
                        <div class="ui-table-paging" id="grade_ranking_paging"></div>
                    </div>
                    <!--// .panel__body -->
                </div>
                <!--// .ui-panel -->
...
require.async(['/components/header/header.js', '/components/sidebar/sidebar.js'], function(HeaderMenu, SidebarMenu){
    SidebarMenu.init('statistics', 'grade');
});

var Url = require('url'),
    Paging = require('/components/paging/paging.js');

var Grade_ranking = (function(){
    var page_no = 1,
        page_limit = 10,
        page_total = null,
        records_length = null;
    ...
    function get_resources(_page_next, _class_no, _success_callback){
        ...
        get_resources_handler = $.getJSON('/school/V1/report/_appraisalViewPage', resource_params).done(function(_data, _status, _xhr){
        ...
        $.each(_result, function(__index, __item){
            tpl.push([
                '<tr class="'+ ((__index>0 && (__index%2)) ? 'ui-table-split' : '') +'">',
                    '<td>'+ (__index+1) +'</td>',
                    '<td>'+ __item['date'] +'</td>',
                    '<td>'+ __item['student_name'] +'</td>',
                    '<td>'+ __item['student_code'] +'</td>',
                    '<td>'+ __item['building_name'] + '&nbsp;' + __item['level_name'] + '&nbsp;' + __item['room_name'] +'</td>',
                    '<td>'+ __item['duration'] +'</td>',
                    '<td>'+ __item['rule_name'] +'</td>',
                    '<td>',
                        '<a href="'+ __item['img_url'] +'" class="ui-button ui-button-xs" title="查看图片"><i class="fa fa-image"></i>&nbsp;查看图片</a>',
                    '</td>',
    ...
    return {
        init: function(_$table_id, _params){
            ...
            get_resources(page_no, _params['class_no'], function(_data){
                (!!_data['result'] && _data['result'].length>0) && Paging.init('grade_ranking_paging', {
                    pageNo:  _data['pageNo'],
...
Grade_ranking.init('grade_ranking', {
...
$('#back_to_list').on('click', function(){
    var list_path = Url('?list_path') || null,
        history_back_url = (list_path!==null && list_path.length!==0) ? window.decodeURIComponent(list_path) : '/school/route/statistics/gradeRankingMore';
