    ...
    <div class="body-container">
        ...
        <div class="body-container__main">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-split">/</span>
                <span class="breadcrumb-active">班级熄灯排名报表</span>
            </div>

            <div class="body-content__wrapper">
                <div class="ui-panel">
                    <div class="panel__header">
                        <span class="panel-title"><i class="fa fa-list"></i>&nbsp;班级熄灯排名报表</span>
                    </div>
                    <div class="panel__body">
                        <div class="panel-toolbar">
                            <div class="ui-search">
                                ...
                                <label for="" class="search-label">排序方式：</label>
                                <div class="search__item">
                                    <select name="" id="search_order" class="ui-form-select">
                                        <option value="0">默认</option>
                                        <option value="1">倒序</option>
                                    </select>
                                </div>
                                <a href="javascript:void(0);" class="ui-button ui-button-sm search__item_last" id="search_submit"><i class="fa fa-search"></i>&nbsp;统计</a>
                            </div>
                        </div>
                        <div class="ui-table-container">
                            ...
                            <table class="ui-table" id="lights_out_ranking">
                                <thead>
                                    <tr>
                                        <th style="width:46px;">排名</th>
                                        <th>班级</th>
                                        <th>班主任</th>
                                        <th style="width:180px">违规时长（分钟）</th>
                        ...
                        <div class="ui-table-paging" id="lights_out_ranking_paging"></div>
                    </div>
                    <!--// .panel__body -->
                </div>
                <!--// .ui-panel -->
...
require.async(['/components/header/header.js', '/components/sidebar/sidebar.js'], function(HeaderMenu, SidebarMenu){
    SidebarMenu.init('statistics', 'lights');
});
...
var LightsOut_ranking = (function(){
    var page_no = 1,
        page_limit = 10,
        page_total = null,
        records_length = null;
    ...
    function get_resources(_page_next, _begin_date, _end_date, _order, _success_callback){
        ...
        resource_params['order'] = !_order ? 0 : parseInt(_order, 10);

        get_resources_handler = $.getJSON('/school/V1/report/_lightsOutPage', resource_params).done(function(_data, _status, _xhr){
        ...
        $.each(_result, function(__index, __item){
            tpl.push([
                '<tr class="'+ ((__index>0 && (__index%2)) ? 'ui-table-split' : '') +'">',
                    ...
                    '<td>',
                        '<a href="/school/route/statistics/lightsOutRankingMoreDetail?class_no='+ __item['class_no'] +'&list_path='+ window.encodeURIComponent(Url())+'" class="ui-button ui-button-xs" title="查看明细"><i class="fa fa-th"></i>&nbsp;查看明细</a>',
                    '</td>',
                '</tr>',
            ].join(''));
        });
    ...
    return {
        init: function(_$table_id, _params){
            ...
            get_resources(page_no, _params['begin_date'], _params['end_date'], _params['order'], function(_data){
                (!!_data['result'] && _data['result'].length>0) && Paging.init('lights_out_ranking_paging', {
                    pageNo:  _data['pageNo'],
...
LightsOut_ranking.init('lights_out_ranking', {
...
$('#search_submit').on('click', function(){
    ...
    var params = {
        'order': parseInt($search_order.val(), 10)
    };
